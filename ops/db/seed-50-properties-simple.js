// Run with: docker exec -i million-mongodb-dev mongosh "mongodb://localhost:27017/million" < ops/db/seed-50-properties-simple.js
...
for (let i = 1; i <= 50; i++) {
  ...
  const price = 500000 + (i * 250000) + (Math.floor(Math.random() * 1000000));
  const size = 1500 + (i * 100) + (Math.floor(Math.random() * 2000));
  const bedrooms = 2 + (i % 4);
  const bathrooms = 2 + (i % 3);
  
  // Generate media array
  const mediaCount = 3 + (i % 8);
  const media = [];
  ...
  // Generate traces (1/3 of properties have traces)
  const traces = [];
  if (i % 3 === 0) {
    const previousPrice = Math.round(price * (0.8 + Math.random() * 0.3));
    const tax = Math.round(previousPrice * 0.05);
    traces.push({
      id: `trace-${i}`,
      dateSale: new Date(Date.now() - (Math.random() * 5 * 365 * 24 * 60 * 60 * 1000)),
      name: "Previous Sale",
      value: previousPrice,
      tax: tax
    });
